...
function searchRes(page_id){
  var dataset = {
    kw: $('#kw').val(),
    id: <?php echo htmlspecialchars($group['id'], ENT_QUOTES, "UTF-8"); ?>,
    page: page_id,
  }; 
    
  var url;
  url = "<?php echo url(array('m'=>$MOD, 'c'=>'cellgroups', 'a'=>'members', 'step'=>'search', ));?>";
 ...
 //     set_selectAll();
      ...
      $('#rows').append("<div class='nors mt5'>未找到小组成员记录...</div>");
    }
  });
}
//翻页
...
function addMember(){
    window.location.href = "<?php echo url(array('m'=>$MOD, 'c'=>'cellgroups', 'a'=>'addMember', ));?>";
}
</script>
</head>
<body>
<div class="content">
  <div class="loc"><h2><i class="icon"></i><?php echo htmlspecialchars($group['name'], ENT_QUOTES, "UTF-8"); ?></h2></div>
  <div class="box">
    <?php if (!$readonly) : ?>
    <div class="doacts">
      <a class="ae add btn" onclick="addMember()"><i></i><font>添加小组成员</font></a>
      <a class="ae btn" onclick="doslvent('<?php echo url(array('m'=>$MOD, 'c'=>'cellgroups', 'a'=>'editMember', ));?>', 'id')"><i class="edit"></i><font>修改成员信息</font></a>
    </div>
    <?php endif; ?>
...
<table class="datalist">
  ...
  {@each list as v}
  <tr>
    <td width="20"><input name="id" type="checkbox" value="${v.member_id}" /></td>
    <td width="40">${v.member_id}</td>
    <td >
      <?php if (!$readonly) : ?>
      <a href="<?php echo url(array('m'=>$MOD, 'c'=>'cellgroups', 'a'=>'editMember', ));?>&id=${v.member_id}"><font class="blue">${v.name}</font></a>
      <?php else : ?>
      ${v.name}
      <?php endif; ?>
    </td>
    ...
    <td>
      {@if v.house_head==1}Yes{@else}No{@/if}
    </td>
  </tr>
  {@/each}
</table>
