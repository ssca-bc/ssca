...
<body>
<?php if ($_GET['a'] == 'addMember') : ?>
<div class="content">
  <div class="loc"><h1><i class="icon"></i>添加细胞小组成员</h1></div>
  <form method="post" action="<?php echo url(array('m'=>$MOD, 'c'=>'cellgroups', 'a'=>'addMember', 'step'=>'submit', ));?>">
            ...
            <td>
              <select name="head_id">
                ...
                <?php $_foreach_head_counter = 0; $_foreach_head_total = count($heads);?><?php foreach( $heads as $head ) : ?><?php $_foreach_head_index = $_foreach_head_counter;$_foreach_head_iteration = $_foreach_head_counter + 1;$_foreach_head_first = ($_foreach_head_counter == 0);$_foreach_head_last = ($_foreach_head_counter == $_foreach_head_total - 1);$_foreach_head_counter++;?>
                  <option value="<?php echo htmlspecialchars($head['member_id'], ENT_QUOTES, "UTF-8"); ?>"><?php echo htmlspecialchars($head['name'], ENT_QUOTES, "UTF-8"); ?></option>
                <?php endforeach; ?>
              </select>
            </td>
          </tr>
...
<div class="content">
  ...
  <form method="post" action="<?php echo url(array('m'=>$MOD, 'c'=>'cellgroups', 'a'=>'editMember', 'step'=>'submit', ));?>">
    <input type="hidden" name="id" value="<?php echo htmlspecialchars($member['member_id'], ENT_QUOTES, "UTF-8"); ?>" />
    <input type="hidden" name="<?php echo htmlspecialchars($_SESSION['ADMIN_FORM_TOKEN']['KEY'], ENT_QUOTES, "UTF-8"); ?>" value="<?php echo htmlspecialchars($_SESSION['ADMIN_FORM_TOKEN']['VAL'], ENT_QUOTES, "UTF-8"); ?>" />
    <div class="box">
      <div class="module">
        <table class="dataform">
          <tr>
            <th width="300">姓名/Name</th>
            <td><input class="w200 txt" name="name" id="name" type="text" value="<?php echo htmlspecialchars($member['name'], ENT_QUOTES, "UTF-8"); ?>" /><p class="c999 mt10">Please fill in member's name</p></td>
          </tr>
          <tr>
            <th>户主/Head of House</th>
            <td>
                <input type="checkbox" name="house_head" id="house_head" value="1" <?php if ($member['house_head'] == 1) : ?>checked<?php endif; ?> /> Is head of house ?
                <p class="c999 mt10">If this member is head of house, please check</p>
            </td>
          </tr>
          <tr id="tr_head_id" <?php if ($member['house_head'] == 1) : ?>class="hide"<?php endif; ?>>
            <th>选择户主/Choose Head of House</th>
            <td>
              <select name="head_id">
                <?php $_foreach_head_counter = 0; $_foreach_head_total = count($heads);?><?php foreach( $heads as $head ) : ?><?php $_foreach_head_index = $_foreach_head_counter;$_foreach_head_iteration = $_foreach_head_counter + 1;$_foreach_head_first = ($_foreach_head_counter == 0);$_foreach_head_last = ($_foreach_head_counter == $_foreach_head_total - 1);$_foreach_head_counter++;?>
                  <option value="<?php echo htmlspecialchars($head['member_id'], ENT_QUOTES, "UTF-8"); ?>" <?php if ($member['head_id'] == $head['member_id']) : ?>selected<?php endif; ?>><?php echo htmlspecialchars($head['name'], ENT_QUOTES, "UTF-8"); ?></option>
                <?php endforeach; ?>
              </select>
            </td>
          </tr>
          <tr id="tr_phone" <?php if ($member['house_head'] == 0) : ?>class="hide"<?php endif; ?>>
            <th>电话/Phone</th>
            <td><input class="w200 txt" name="phone" id="phone" type="text" value="<?php echo htmlspecialchars($member['phone'], ENT_QUOTES, "UTF-8"); ?>" /><p class="c999 mt10">Please fill in member's phone number</p></td>
          </tr>
          <tr id="tr_email" <?php if ($member['house_head'] == 0) : ?>class="hide"<?php endif; ?>>
            <th>电邮/Email</th>
            <td><input class="w200 txt" name="email" id="email" type="text" value="<?php echo htmlspecialchars($member['email'], ENT_QUOTES, "UTF-8"); ?>" /><p class="c999 mt10">Please fill in member's email</p>
            </td>
          </tr>
          <tr id="tr_address" <?php if ($member['house_head'] == 0) : ?>class="hide"<?php endif; ?>>
            <th>住址/Address</th>
            <td><input class="w200 txt" name="address" id="address" type="text" value="<?php echo htmlspecialchars($member['address'], ENT_QUOTES, "UTF-8"); ?>" /><p class="c999 mt10">Please fill in member's address</p>
            </td>
          </tr>
        </table>
      </div>
